/*
    So, at first I need to convert ACII characters into binary digits. And then I need to divide
plaintText into blocks of 64 bits length. So then If the length either of a block or a key will not be a multiple of 64
then I need to supplement it with zeros in the end. 

129 ConvertTextToBits

The approach of converting numbers/binaries to binaries/numbers
    
let binary25 = (25).toString(2);
console.log(binary25);
let decimal25 = parseInt(binary25, 2);
console.log(decimal25);
...
/// There may be a mistake related to the zeroes appending!
...
const sixteenRounds = (lptBits, rptBits, discardedKeyBits) => {
  ...
  for (let i = 0; i < 16; i++) {
    ...
    let compressedKeyBits = compressionPermutation(shiftedKeyBits);

    let compressedKey = compressedKeyBits.join(",");

    let expandedRptBits = expansionPermutation(rptBits);

    let expandedRpt = expandedRptBits.join(",");

    let XORedRptBits = xorOperation(compressedKeyBits, expandedRptBits);

    let xoredRpt = XORedRptBits.join(",");

    let sBoxRptBits = sBoxSubstitution(XORedRptBits);

    let sboxPrt = sBoxRptBits.join(",");

    let pBoxRptBits = pBoxPermutation(sBoxRptBits);

    let pBoxRpt = pBoxRptBits.join(",");

    let XORedTextBits = xorOperation(pBoxRptBits, lptBits);

    lptBits = rptBits;
    rptBits = XORedTextBits;

    let xoredText = XORedTextBits.join(",");
    let lpt = lptBits.join("");
    let rpt = rptBits.join("");

    if (i === 3) {
      console.log(
        `iteration (1) --> ${i} RESULT XOR ASSIGNED TO RPT --> ${xoredText}`
      );
      console.log(`compressedKey --> ${compressedKey}`);
      console.log(`expandedRpt --> ${expandedRpt}`);
      console.log(`xoredRpt --> ${xoredRpt}`);
      console.log(`sboxPrt --> ${sboxPrt}`);
      console.log(`pBoxRpt --> ${pBoxRpt}`);
      console.log(`final lpt --> ${lpt}`);
      console.log(`final rpt --> ${rpt}`);
    }
  }
...
// Main function for encrypting textBitBlocks
const encryptBitBlock = (textBits, discardedKeyBits) => {
  let initiallyPermutatedTextBits = initialPermutation(textBits);
...
export const encrypt = (plainText, key) => {
  // converting plainText characters into binary digits
  let textBits = convertTextToBits(plainText);

  // converting key charactersIntoBinaryDigist
  let keyBits = convertTextToBits(key);

  // Zeroes supplemention
  let appendedTextBits = appendZeroes(textBits);
  let appendedKeyBits = appendZeroes(keyBits);

  // discard every 8th bit from the original key array
  let discardedKeyBits = discardEighthBitsFromKey(appendedKeyBits);

  let cipherTextBits = [];

  for (let i = 0; i < appendedTextBits.length; i += 64) {
    let textBitSlice = appendedTextBits.slice(i, i + 64);
    cipherTextBits.push(...encryptBitBlock(textBitSlice, discardedKeyBits));
  }

  console.log(cipherTextBits.join(""));
};
